...
function submitOrder() {
    ...
    counts.forEach(function (countSpan) {
        ...
        if (count > 0) {
            ...
            var url = `https://fastfood-toi.uz/api/user/addfoods/?user_id=${userId}&code=${code}&count=${count}`;
    ...
    Promise.all(requests).then(() => {
        ...
        addToCartButton.style.display = 'none';
        onaddToCartButton.style.display = 'flex';
    });
}
...
if (userId) {
    ...
    links.forEach(link => {
        ...
        document.getElementById('cont').classList.remove('blov');
} else {
